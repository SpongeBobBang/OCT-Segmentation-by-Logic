...
class KmeansClustering():
    ...
    """
    def __init__(self, name_img, map_features, k, max_num_iteration, epsilon_change=0.01, num_attempt=5):
    ...
    def get_criteria(self):
        ...
        """
        return cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, self.max_num_iteration, self.epsilon_change
    def cluster(self):
        """ Perform the clustering """
        return cv2.kmeans(self.samples, self.k, None, self.criteria, self.num_attempt, cv2.KMEANS_RANDOM_CENTERS)
...
def cluster_output_image_label(name_img, map_features, k, max_num_iteration, epsilon_change=0.01, num_attempt=5):
    """ Write a image file of labels, given k-means clustering parameters """
    clustering = KmeansClustering(name_img, map_features, k, max_num_iteration, epsilon_change, num_attempt)
    clustering.output_image_label()

if __name__ == "__main__":
    NAME = "Stefan with Art.jpg"
    CLUSTER = KmeansClustering(NAME, cv2.imread(NAME), 4, 10)
