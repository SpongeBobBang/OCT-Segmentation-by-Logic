...
def videofig(num_frames, redraw_func, play_fps=25, big_scroll=30, key_func=None, *args):
    ...
    axes_handle = plt.axes([0.1, 0.3, 1, 0.97])
    axes_handle.set_axis_off()

    # Build scrollbar
    scroll_axes_handle = plt.axes(
        [0, 0, 1, 0.03], facecolor='lightgoldenrodyellow')
    scroll_handle = Slider(scroll_axes_handle, '', 0.0,
                           num_frames - 1, valinit=0.0)
    ...
    def scroll(new_f):
        # clip in the range of [0, num_frames - 1]
...
def check_callback(a, name):
    ...
    assert callable(a), '{} must be callable, instead of {}'.format(
        name, type(name))
...
if __name__ == '__main__':
    ...
    video_dir = 'D:/UMD/Career/Research Assistant/Segmentation by Logic/Code/Image_Kidney/ori'
